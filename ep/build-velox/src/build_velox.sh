...
function compile {
    ...
    COMPILE_OPTION="-DTREAT_WARNINGS_AS_ERRORS=OFF -DDVELOX_ENABLE_PARQUET=ON -DVELOX_BUILD_TESTING=OFF -DVELOX_ENABLE_DUCKDB=OFF -DVELOX_BUILD_TEST_UTILS=ON -DVELOX_CODEGEN_SUPPORT=OFF"
    if [ $ENABLE_HDFS == "ON" ]; then
